...
parseCanonicalJson
  :: (MessagePackObject a, MessageUnpackObject a)
  => JS.Value -> JS.Parser a
...
jsonFromMsgpack (MP.ObjectMap items) = JS.Object $ HM.fromList $ map pair items
  where
    ...
    pair (k, v) = case MP.fromObject k of
      EitherError (Left err) -> error $
        "Can only work with maps from text keys: " <> err
...
msgpackFromJson (JS.Object o) = MP.ObjectMap . map mapItem . HM.toList $ o
  where
    ...
    mapItem ("snd", JS.String (decodeAddressJson -> Just mp)) =
      (MP.ObjectStr "snd", mp)
    mapItem ("rcv", JS.String (decodeAddressJson -> Just mp)) =
      (MP.ObjectStr "rcv", mp)
    -- Default case
